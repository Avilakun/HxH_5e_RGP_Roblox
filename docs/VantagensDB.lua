-- VantagensDB.lua -- gerado automaticamente a partir de config.js
-- (chave "inclinacoes") do webapp (github.com/Criadores-HxH-5e/
-- Ficha_HxH5e). NAO CONFUNDIR com CombatInclinationsDB.lua (que ja
-- existe, chave "COMBAT_INCLINATIONS_DB" -- sistema diferente, de
-- trilhas de combate). Essa aqui e um sistema de Vantagens e
-- Desvantagens tipo GURPS, comprado com pontos na criacao do
-- personagem.
--
-- .positivas (27): vantagens, cada uma com um "custo" em pontos.
-- .negativas (42): desvantagens, cada uma com um "valor" em pontos
--   que o jogador GANHA de volta pra gastar em vantagens (convencao
--   inversa a positivas -- valor alto = desvantagem mais grave).
--
-- hasOptions=true + options={...}: a vantagem/desvantagem tem
-- variantes escolhiveis (cada uma com seu proprio label/custo-valor/
-- desc), em vez de um efeito unico fixo.
local VantagensDB = {
	positivas = {
		{
			nome = "Aliado",
...
local porNome = {}
for _, v in ipairs(VantagensDB.positivas) do
	porNome[v.nome] = v
end
for _, v in ipairs(VantagensDB.negativas) do
	porNome[v.nome] = v
end

function VantagensDB.FindByNome(nome)
	return porNome[nome]
end
