-- CombatInclinationsDB.lua -- gerado automaticamente a partir de
-- combat-inclinations-db.js do webapp (github.com/Criadores-HxH-5e/Ficha_HxH5e).
-- 24 inclinacoes de combate, cada uma com 3 tiers progressivos (o
-- personagem escolhe UMA inclinacao e evolui pelos 3 niveis dela).
-- Sistema nunca tinha sido portado pro jogo -- so a base de dados aqui,
-- ainda sem UI nem conexao com CharacterService pra atribuir/aplicar.

local CombatInclinationsDB = {}

CombatInclinationsDB.Inclinacoes = {
	{
		id = "analitica",
		nome = "Analítica",
		tiers = {
			"+2 Reações.",
			"-1 Reação para adicionar 2 no acerto do seu próximo ataque (Livre por gasto para um novo alvo).",
			"+1 em INT e +1 em SAB.",
		},
		attrBonusFixo = {
			{ attr = "INT", valor = 1 },
			{ attr = "SAB", valor = 1 },
		},
	},
	{
		id = "arsenal_militar",
		nome = "Arsenal Militar",
		tiers = {
			"Te dá acesso a armas simples científicas.",
			"Te dá acesso a armas complexas científicas.",
			"Te dá acesso a armas especiais científicas e de cerco.",
		},
	},
	{
		id = "conjurador_de_guerra",
		nome = "Conjurador de Guerra",
		tiers = {
			"Vantagem em TRs de Constituição para manter concentração em conjuração ao sofrer dano.",
			"Pode aplicar IN em conjurações como: (a) ação livre no turno; ou (b) por metade da aura.",
			"+2 em INT.",
		},
		attrBonusFixo = {
			{ attr = "INT", valor = 2 },
		},
	},
	{
		id = "combate_montado",
		nome = "Combate Montado",
		tiers = {
			"Vantagem em ataques corpo-a-corpo contra alvos desmontados menores que sua montaria/veículo.",
			"Reação para redirecionar ataque da montaria para você ou vice-versa; atacante refaz a jogada.",
			"Montaria/veículo recebe +2 de CA ou no dano quando conduzido por você.",
		},
	},
	{
		id = "defensiva_bruta",
		nome = "Defensiva Bruta",
		tiers = {
			"3 em Redução de Dano (RD) contra qualquer fonte de dano de qualquer oponente em combate.",
			"3×CON em RD em um ataque que falhe em bloquear (um uso por rodada).",
			"+2 em CON.",
		},
		attrBonusFixo = {
			{ attr = "CON", valor = 2 },
		},
	},
	{
		id = "defensiva_precisa",
		nome = "Defensiva Precisa",
		tiers = {
			"5 em Redução de Dano (RD) contra qualquer fonte de dano de um alvo em combate.",
			"+2 na CA ao usar Bloqueio Armado (um uso por rodada).",
			"+2 em CON.",
		},
		attrBonusFixo = {
			{ attr = "CON", valor = 2 },
		},
	},
	{
		id = "defensiva_reativa_agil",
		nome = "Defensiva Reativa-Ágil",
		tiers = {
			"Defender como reação (Bloqueio) não gasta reação.",
			"Dobra proficiência na esquiva ou esquiva acrobática (um uso por rodada).",
			"+2 em CON.",
		},
		attrBonusFixo = {
			{ attr = "CON", valor = 2 },
		},
	},
	{
		id = "experiente",
		nome = "Experiente",
		tiers = {
			"Registra características de combate de inimigos engajados (ação principal; ação bônus se Cientista/Técnico).",
			"Adiciona 1d6 (+prof) contra todos os aspectos analisados de um inimigo já analisado.",
			"Escolha: (1) dobra prof. da análise; (2) analisa +1 inimigo por rodada; ou (3) imune a Intimidação e Enganação de alvos analisados.",
		},
	},
	{
		id = "imobilizador",
		nome = "Imobilizador",
		req = "FOR 16+",
		tiers = {
			"Realiza ataque de agarrão contra a CA do alvo em vez de teste contestado.",
			"Reação para agarrar de volta um alvo preso que tente escapar.",
			"+2 em FOR.",
		},
		attrBonusFixo = {
			{ attr = "FOR", valor = 2 },
		},
	},
	{
		id = "investida_poderosa",
		nome = "Investida Poderosa",
		req = "FOR 16+",
		tiers = {
			"Ao usar ação Disparada, pode usar Ação Bônus para atacar corpo-a-corpo ou empurrar.",
			"Se o ataque/empurrão for bem-sucedido: (a) empurre o alvo 4m; ou (b) +5 no dano.",
			"+2 em FOR.",
		},
		attrBonusFixo = {
			{ attr = "FOR", valor = 2 },
		},
	},
	{
		id = "maestria_armadura_media",
		nome = "Maestria em Armadura Média",
		req = "Prof. em Armadura Média",
		tiers = {
			"Armadura média não impõe desvantagem, prejuízo ou limitação.",
			"Pode adicionar até 4 de DES à CA em vez do máximo 3.",
			"Aumenta a durabilidade da armadura média em 2.",
		},
	},
	{
		id = "maestria_armadura_pesada",
		nome = "Maestria em Armadura Pesada",
		req = "Prof. em Armadura Pesada",
		tiers = {
			"Eficiência da armadura aumenta em 2 (bônus à CA).",
			"Acertos que igualam à CA recebem 3 de redução de dano.",
			"Aumenta a durabilidade da armadura pesada em 2.",
		},
	},
	{
		id = "mestre_de_armas",
		nome = "Mestre de Armas",
		tiers = {
			"Ganha proficiência com qualquer arma.",
			"Qualquer item em sua posse torna-se arma (1d6 mínimo); imune a ser desarmado.",
			"+1 em FOR ou +1 em DES.",
		},
	},
	{
		id = "mestre_de_armas_grandes",
		nome = "Mestre de Armas Grandes",
		req = "FOR 16+",
		tiers = {
			"Em acerto crítico ou ao reduzir alvo a 0 PV, Ação Bônus para ataque extra corpo-a-corpo (uma vez por rodada).",
			"Antes de atacar com arma pesada: –5 no ataque e +10 no dano.",
			"Armas com Alcance ou Duas Mãos recebem propriedade Mortal ×3.",
		},
	},
	{
		id = "mestre_de_escudo",
		nome = "Mestre de Escudo",
		tiers = {
			"Adiciona bônus de CA do escudo em TRs de Destreza que tenham você como alvo.",
			"Reação: não sofre dano se passar no TR de DES contra efeito de metade do dano (um uso por rodada).",
			"+1 em CON e +1 em FOR.",
		},
		attrBonusFixo = {
			{ attr = "CON", valor = 1 },
			{ attr = "FOR", valor = 1 },
		},
	},
	{
		id = "ofensiva_bruta",
		nome = "Ofensiva Bruta",
		tiers = {
			"–2 na CA e +2 graus em qualquer ataque corpo-a-corpo com FOR.",
			"–3 Reações para dobrar o mod de FOR no dano do próximo ataque (um uso por rodada).",
			"+2 em FOR.",
		},
		attrBonusFixo = {
			{ attr = "FOR", valor = 2 },
		},
	},
	{
		id = "ofensiva_precisa",
		nome = "Ofensiva Precisa",
		tiers = {
			"–2 na CA e +2 graus nos ataques à distância/arremesso.",
			"+2 no acerto de um ataque à distância se não se mover na rodada.",
			"+2 em DES, SAB ou INT.",
		},
	},
	{
		id = "ofensiva_agil",
		nome = "Ofensiva Ágil",
		tiers = {
			"+5 na iniciativa.",
			"–2 no acerto de um contra-ataque do inimigo (um uso por rodada).",
			"+2 em DES, SAB ou INT.",
		},
	},
	{
		id = "perceptiva",
		nome = "Perceptiva",
		tiers = {
			"+2 na iniciativa.",
			"Percepção passiva +5 em combate; pode usar GYO sem gastar ação (ao custo do movimento, metade da aura) se não se mover.",
			"+2 em SAB.",
		},
		attrBonusFixo = {
			{ attr = "SAB", valor = 2 },
		},
	},
	{
		id = "premeditada_manipulada",
		nome = "Premeditada (Manipulada)",
		tiers = {
			"Escolha: (1) alvos manipulados +1 grau no passo de dano; ou (2) alvos manipulados +2 na CA.",
			"Adiciona 1d4 em uma jogada (não 1d20) para o alvo da manipulação (um uso por rodada).",
			"+1 em INT ou CAR.",
		},
	},
	{
		id = "premeditada_planejada",
		nome = "Premeditada (Planejada)",
		tiers = {
			"Item/constructo feito por você fica com IN de forma passiva (sem gastar aura nem ação).",
			"Item/constructo feito por você adiciona proficiência no ataque/defesa ou TR provocado.",
			"+2 em INT.",
		},
		attrBonusFixo = {
			{ attr = "INT", valor = 2 },
		},
	},
	{
		id = "sentinela",
		nome = "Sentinela",
		tiers = {
			"Provoca ataques de oportunidade mesmo que o inimigo use Desengajar.",
			"Ao acertar com ataque de oportunidade, deslocamento do alvo é reduzido à metade pelo resto do turno.",
			"+1 em DES e +1 em SAB.",
		},
		attrBonusFixo = {
			{ attr = "DES", valor = 1 },
			{ attr = "SAB", valor = 1 },
		},
	},
	{
		id = "trapaceira",
		nome = "Trapaceira",
		tiers = {
			"Escolha: (1) golpes desarmados ganham Finta/Oculto; (2) vantagem em Enganação/Persuasão em batalha; ou (3) +3 em Furtividade/Prestidigitação em batalha.",
			"Uma vez por rodada: finta ou prestidigitação (TR 10) como Ação Bônus para atacar/pegar item sem ser visto.",
			"+2 em DES ou CAR.",
		},
	},
	{
		id = "visao_360",
		nome = "Visão 360°",
		tiers = {
			"Campo de visão 360°; imune a flanqueamento; não pode ser surpreendido.",
			"Uma vez por rodada: vantagem e +3 no TR de Percepção/Intuição contra Oculto/Finta de ataque recebido.",
			"+2 em SAB.",
		},
		attrBonusFixo = {
			{ attr = "SAB", valor = 2 },
		},
	},
}

local porId = {}
for _, i in ipairs(CombatInclinationsDB.Inclinacoes) do
	porId[i.id] = i
end

function CombatInclinationsDB.FindById(id)
	return porId[id]
end

return CombatInclinationsDB
